...
namespace GirbalPathfinding
{
    public class AStar : IPathPlannable //changed from internal to public
    {
        ...
        public AStar(State startState, State goalState, int agentId)
        {
        ...
        public void InitialisePathPlanner(Map mp, int[,] staticObs)
        {
        ...
        public List<State> PlanPath(List<(State, int)> newConstraints)
        {
            ...
            if (!startState.positionEqual(goalState, false))
            ...
            while (!state.positionEqual(goalState, false))
            {
                path.Add(state);
